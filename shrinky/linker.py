...
class Linker:
    ...
    def generate_linker_flags(self):
        ...
        if self.__command_basename.startswith(
            "g++"
        ) or self.__command_basename.startswith("gcc"):
            self.__linker_flags += [
                "-nostartfiles",
                "-nostdlib",
                "-Xlinker",
            ...
            if dynamic_linker.startswith('"') and dynamic_linker.endswith('"'):
                ...
                raise RuntimeError(
                    "dynamic liner definition '%s' should be quoeted" % (dynamic_linker)
                )
            self.__linker_flags += [
                "-nostdlib",
                "--strip-all",
        ...
        for ii in self.__library_directories:
            ...
            if file_is_ascii_text(current_libname):
                ...
                if match:
                    ...
                    if is_verbose():
                        print(
                            "Using shared library '%s' instead of '%s'."
                            % (ret, libname)
                        )
                    return ret
    ...
    def generate_linker_script(self, dst, modify_start=False):
        ...
        (so, se) = run_command(
            [self.__command, "--verbose"] + self.__linker_flags_extra
        )
        ...
        ld_script = re.sub(
            ...
            r"SEGMENT_START(\1, %s)" % (str(PlatformVar("entry"))),
            ld_script,
            re.MULTILINE,
        )
        if modify_start:
            ld_script = re.sub(
                r"(SEGMENT_START.*\S)\s*\+\s*SIZEOF_HEADERS\s*;",
                r"\1;",
                ld_script,
                re.MULTILINE,
            )
    ...
    def link(self, src, dst, extra_args=[]):
        ...
        cmd = (
            ...
            + self.__linker_flags
            + self.get_library_directory_list()
