...
from shrinky.assembler import Assembler
from shrinky.assembler_file import AssemblerFile
from shrinky.assembler_segment import AssemblerSegment
from shrinky.common import executable_find
from shrinky.common import executable_search
from shrinky.common import get_indent
from shrinky.common import is_listing
from shrinky.common import is_verbose
from shrinky.common import listify
from shrinky.common import locate
from shrinky.common import run_command
from shrinky.common import set_verbose
from shrinky.compiler import Compiler
from shrinky.custom_help_formatter import CustomHelpFormatter
from shrinky.glsl import Glsl
from shrinky.library_definition import g_library_definitions
from shrinky.linker import Linker
from shrinky.platform_var import g_osarch
from shrinky.platform_var import g_osname
from shrinky.platform_var import osarch_is_amd64
from shrinky.platform_var import osarch_is_32_bit
from shrinky.platform_var import osarch_is_64_bit
from shrinky.platform_var import PlatformVar
from shrinky.platform_var import platform_map_iterate
from shrinky.platform_var import replace_osarch
from shrinky.platform_var import replace_osname
from shrinky.platform_var import replace_platform_variable
from shrinky.preprocessor import Preprocessor
from shrinky.symbol import generate_loader_dlfcn
from shrinky.symbol import generate_loader_hash
from shrinky.symbol import generate_loader_vanilla
from shrinky.symbol import generate_symbol_definitions_direct
from shrinky.symbol import generate_symbol_definitions_table
from shrinky.symbol import generate_symbol_table
from shrinky.symbol_source_database import g_symbol_sources
from shrinky.template import Template
...
g_assembler_phdr32_load_double = (
    ...
    ("p_filesz, program size on disk", PlatformVar("addr"), "end - ehdr"),
    (
        "p_memsz, program headers size in memory",
        PlatformVar("addr"),
        "aligned_end - ehdr",
    ),
    ("p_flags, rwx = 7", 4, 7),
    (
        "p_align, usually " + str(PlatformVar("memory_page")),
        PlatformVar("addr"),
        PlatformVar("memory_page"),
    ),
)
...
g_template_header = Template("""#ifndef SHRINKY_H
#define SHRINKY_H\n
/** \\file
...
#endif
/** Program entry point. */
void _start() SHRINKY_VISIBILITY;[[UND_SYMBOLS]]
#if defined(__cplusplus)
}
#endif
#endif
...
#include \"[[HEADER_RAND]]\"
#else
#define SHRINKY_RAND_IMPLEMENTATION_BSD [[RAND_TYPE_BSD]]
#define SHRINKY_RAND_IMPLEMENTATION_GNU [[RAND_TYPE_GNU]]
#if defined(__FreeBSD__)
#if !SHRINKY_RAND_IMPLEMENTATION_BSD
#include \"[[HEADER_RAND]]\"
#include \"[[SOURCE_RAND]]\"
#else
...
#endif
#elif defined(__linux__)
#if !SHRINKY_RAND_IMPLEMENTATION_GNU
#include \"[[HEADER_RAND]]\"
#include \"[[SOURCE_RAND]]\"
#else
...
def collect_libraries(libraries, symbols, compilation_mode):
    ...
    if not libraries:
        if "dlfcn" == compilation_mode:
            raise RuntimeError(
                "cannot autodetect libraries for compilation mode '%s'"
                % compilation_mode
            )
        library_set = set()
        for ii in symbols:
        ...
        if missing_libraries:
            print(
                "WARNING: found symbols suggest libraries: %s"
                % (str(list(missing_libraries)))
            )
...
def compress_file(compression, pretty, src, dst):
    ...
    if "lzma" == compression:
        command = [
            "xz",
            "--format=lzma",
            "--lzma1=preset=9,lc=1,lp=0,nice=273,pb=0",
            "--stdout",
        ]
        header = "#!/bin/sh\nHOME=/tmp/i;%s $0|xz -dFlzma>~;chmod +x ~;~%s" % (
            str_tail,
            str_cleanup,
        )
    elif "raw" == compression:
        command = ["xz", "-9", "--extreme", "--format=raw", "--stdout"]
        header = "#!/bin/sh\nHOME=/tmp/i;%s $0|xzcat -F raw>~;chmod +x ~;~%s" % (
            str_tail,
            str_cleanup,
        )
    elif "xz" == compression:
        command = [
            "xz",
            "--format=xz",
            "--lzma2=preset=9,lc=1,nice=273,pb=0",
            "--stdout",
        ]
        header = "#!/bin/sh\nHOME=/tmp/i;%s $0|xzcat>~;chmod +x ~;~%s" % (
            str_tail,
            str_cleanup,
        )
    else:
...
def generate_binary_minimal(
    ...
    if asm.write(output_file + ".S", assembler):
        ...
        additional_file = g_symbol_sources.compile_asm(
            compiler, assembler, extra_symbols, output_file + ".extra"
        )
        # If additional code was needed, add it to our asm source.
        if additional_file:
            additional_asm = AssemblerFile(additional_file)
            asm.incorporate(
                additional_asm, re.sub(r"[\/\.]", "_", output_file + "_extra")
            )
    ...
    segments = (
        merge_segments(segments_head) + load_segments + merge_segments(segments_tail)
    )
...
def generate_elfling(output_file, compiler, elfling, definition_ld):
    ...
    if elfling.has_data():
        alignment_section = AssemblerSectionAlignment(
            elfling_align, ELFLING_PADDING, ELFLING_OUTPUT, "end"
        )
        set_program_start("_start")
    else:
        alignment_section = AssemblerSectionAlignment(
            elfling_align, ELFLING_PADDING, ELFLING_OUTPUT
        )
...
def generate_glsl(
    filenames, preprocessor, definition_ld, mode, inlines, renames, simplifys
):
...
def generate_glsl_extract(
    fname, preprocessor, definition_ld, mode, inlines, renames, simplifys
):
    ...
    filenames = []
    glslre = re.compile(
        r"#\s*include [\<\"](.*\.glsl)\.(h|hh|hpp|hxx)[\>\"]\s*(\/\*|\/\/)\s*([^\*\/\s]+)",
        re.I,
    )
    for ii in lines:
        ...
        if match:
            ...
            if not glsl_filename:
                raise RuntimeError(
                    "could not locate GLSL source '%s'" % (glsl_base_filename)
                )
...
def generate_include_rand(implementation_rand, target_search_path, definition_ld):
    ...
    regex_rand_header = re.compile(
        r"%s[-_\s]+rand\.h(h|pp|xx)?" % (implementation_rand)
    )
    regex_rand_source = re.compile(
        r"%s[-_\s]+rand\.c(c|pp|xx)?" % (implementation_rand)
    )
    ...
    if (not header_rand) or (not source_rand):
        raise RuntimeError(
            "could not find rand implementation for '%s'" % (implementation_rand)
        )
    ...
    return g_template_include_rand.format(
        {
            "DEFINITION_LD": definition_ld,
            "RAND_TYPE_BSD": rand_type_bsd,
            "RAND_TYPE_GNU": rand_type_gnu,
            "HEADER_RAND": header_rand,
            "SOURCE_RAND": source_rand,
        }
    )
...
def readelf_probe(src, dst, size):
    ...
    if size == truncate_size:
        if is_verbose():
            print(
                "Executable size equals PT_LOAD size (%u bytes), no operation necessary."
                % (size)
            )
        shutil.copy(src, dst)
        return None
...
def main():
    ...
    default_assembler_list = ["as"]
    default_compiler_list = ["g++"]
    default_linker_list = ["ld"]
    default_preprocessor_list = ["cpp", "clang-cpp"]
    default_objcopy_list = ["objcopy"]
    default_strip_list = ["strip"]
    default_sstrip_list = ["sstrip"]
    definitions = []
    extra_assembler_flags = []
    extra_compiler_flags = []
    extra_linker_flags = []
    include_directories = [
        PATH_VIDEOCORE + "/include",
        PATH_VIDEOCORE + "/include/interface/vcos/pthreads",
        PATH_VIDEOCORE + "/include/interface/vmcs_host/linux",
    ...
    parser = argparse.ArgumentParser(
        usage="%s [args] <source file(s)> [-o output]" % (program_name),
        description="Size-optimized executable generator for *nix platforms.\nPreprocesses given source file(s) looking for specifically marked function calls, then generates a dynamic loader header file that can be used within these same source files to decrease executable size.\nOptionally also perform the actual compilation of a size-optimized binary after generating the header.",
    ...
    parser.add_argument(
        ...
        "--objcopy",
        default=None,
        help="Try to use given objcopy executable as opposed to autodetect.",
    )
    parser.add_argument(
        "-C",
        "--compiler",
        default=None,
        help="Try to use given compiler executable as opposed to autodetect.",
    )
    parser.add_argument(
        "-d",
        "--definition-ld",
        default="USE_LD",
        help="Definition to use for checking whether to use 'safe' mechanism instead of dynamic loading.\n(default: %(default)s)",
    )
    parser.add_argument(
        "-D",
        "--define",
        default=[],
        action="append",
        help="Additional preprocessor definition.",
    )
    parser.add_argument(
        "-e", "--elfling", action="store_true", help="Use elfling packer if available."
    )
    parser.add_argument(
        "-E",
        "--preprocess-only",
        action="store_true",
        help="Preprocess only, do not generate compiled output.",
    )
    parser.add_argument(
        "-h", "--help", action="store_true", help="Print this help string and exit."
    )
    parser.add_argument(
        "-I",
        "--include-directory",
        default=[],
        action="append",
        help="Add an include directory to be searched for header files.",
    )
    parser.add_argument(
        "--interp",
        default=None,
        type=str,
        help="Use given interpreter as opposed to platform default.",
    )
    parser.add_argument(
        "-k",
        "--linker",
        default=None,
        help="Try to use given linker executable as opposed to autodetect.",
    )
    parser.add_argument(
        "-l",
        "--library",
        default=[],
        action="append",
        help="Add a library to be linked against.",
    )
    parser.add_argument(
        "-L",
        "--library-directory",
        default=[],
        action="append",
        help="Add a library directory to be searched for libraries when linking.",
    )
    parser.add_argument(
        "-m",
        "--method",
        default="maximum",
        choices=("vanilla", "dlfcn", "hash", "maximum"),
        help="Method to use for decreasing output file size:\n\tvanilla:\n\t\tProduce binary normally, use no tricks except unpack header.\n\tdlfcn:\n\t\tUse dlopen/dlsym to decrease size without dependencies to any specific object format.\n\thash:\n\t\tUse knowledge of object file format to perform 'import by hash' loading, but do not break any specifications.\n\tmaximum:\n\t\tUse all available techniques to decrease output file size. Resulting file may violate object file specification.\n(default: %(default)s)",
    )
    parser.add_argument(
        "--march",
        type=str,
        help="When compiling code, use given architecture as opposed to autodetect.",
    )
    parser.add_argument(
        "--nice-exit",
        default=True,
        action="store_true",
        help="Do not use debugger trap, exit with proper system call.",
    )
    parser.add_argument(
        "--nice-filedump",
        action="store_true",
        help="Do not use dirty tricks in compression header, also remove filedumped binary when done.",
    )
    parser.add_argument(
        "--no-glesv2",
        action="store_true",
        help="Do not probe for OpenGL ES 2.0, always assume regular GL.",
    )
    parser.add_argument(
        "--glsl-mode",
        default="full",
        choices=("none", "nosquash", "full"),
        help="GLSL crunching mode.\n(default: %(default)s)",
    )
    parser.add_argument(
        "--glsl-inlines",
        default=-1,
        type=int,
        help="Maximum number of inline operations to do for GLSL.\n(default: unlimited)",
    )
    parser.add_argument(
        "--glsl-renames",
        default=-1,
        type=int,
        help="Maximum number of rename operations to do for GLSL.\n(default: unlimited)",
    )
    parser.add_argument(
        "--glsl-simplifys",
        default=-1,
        type=int,
        help="Maximum number of simplify operations to do for GLSL.\n(default: unlimited)",
    )
    parser.add_argument(
        "--linux",
        default=True,
        action="store_true",
        help="Try to target Linux if not in Linux. Equal to '-O linux'.",
    )
    parser.add_argument(
        "-o",
        "--output-file",
        default=None,
        help="Compile a named binary, do not only create a header. If the name specified features a path, it will be used verbatim. Otherwise the binary will be created in the same path as source file(s) compiled.",
    )
    parser.add_argument(
        "-O",
        "--operating-system",
        help="Try to target given operating system insofar cross-compilation is possible.",
    )
    parser.add_argument(
        "-P",
        "--call-prefix",
        default="shrinky_",
        help="Call prefix to identify desired calls.\n(default: %(default)s)",
    )
    parser.add_argument(
        "--preprocessor",
        default=None,
        help="Try to use given preprocessor executable as opposed to autodetect.",
    )
    parser.add_argument(
        "--rand",
        default="bsd",
        choices=("bsd", "gnu"),
        help="rand() implementation to use.\n(default: %(default)s)",
    )
    parser.add_argument(
        "--rpath",
        default=[],
        action="append",
        help="Extra rpath locations for linking.",
    )
    parser.add_argument(
        "--safe-symtab", action="store_true", help="Handle DT_SYMTAB in a safe manner."
    )
    parser.add_argument(
        "-s",
        "--search-path",
        default=[],
        action="append",
        help="Directory to search for the header file to generate. May be specified multiple times. If not given, searches paths of source files to compile. If not given and no source files to compile, current path will be used.",
    )
    parser.add_argument(
        "-S",
        "--strip-binary",
        default=None,
        help="Try to use given strip executable as opposed to autodetect.",
    )
    parser.add_argument(
        "-t",
        "--target",
        default="shrinky.h",
        help="Target header file to look for.\n(default: %(default)s)",
    )
    parser.add_argument(
        "-u",
        "--unpack-header",
        choices=("lzma", "xz"),
        default=compression,
        help="Unpack header to use.\n(default: %(default)s)",
    )
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
        help="Print more info about what is being done.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=[],
        nargs="*",
        help="Source file(s) to preprocess and/or compile.",
    )
    ...
    # Definitions.
    if args.nice_exit:
        definitions += ["SHRINKY_NO_DEBUGGER_TRAP"]
    if args.safe_symtab:
        definitions += ["SHRINKY_SAFE_SYMTAB_HANDLING"]
    ...
    preprocessor = Preprocessor(
        executable_find(preprocessor, preprocessor_list, "preprocessor")
    )
    ...
    if source_files_glsl:
        if output_file:
            if 1 < len(source_files_glsl):
                raise RuntimeError(
                    "output file '%s' given for multiple glsl files '%s'"
                    % (str(output_file), str(source_files_glsl))
                )
            else:
                source_files_glsl = [[source_files_glsl[0], output_file]]
        if source_files or source_files_additional:
            raise RuntimeError(
                "can not combine GLSL source %s with other source files %s"
    ...
    if args.m32:
        if osarch_is_32_bit():
            print(
                "WARNING: ignoring 32-bit compile, osarch '%s' already 32-bit"
                % (g_osarch)
            )
            ...
            raise RuntimeError(
                "cannot attempt 32-bit compile for osarch '%s'" % (g_osarch)
            )
    if args.march:
        if is_verbose:
    ...
    if not compilation_mode in ("vanilla", "dlfcn", "hash", "maximum"):
        ...
        definitions += ["SHRINKY_NO_FIXED_R_DEBUG_ADDRESS"]

    gles_reason = None
    if not no_glesv2:
        if os.path.exists(PATH_MALI):
            definitions += ["SHRINKY_MALI"]
            gles_reason = "'%s' (Mali)" % (PATH_MALI)
        if os.path.exists(PATH_VIDEOCORE):
            definitions += ["SHRINKY_VIDEOCORE"]
    ...
    if target_path:
        ...
        if target_file:
            ...
            raise RuntimeError(
                "no information where to put header file '%s' - not found in path(s) %s"
                % (target, str(target_search_path))
            )
    ...
    if symbols_has_library(symbols, ("GL", "GLESv2")):
        subst["INCLUDE_OPENGL"] = g_template_include_opengl.format(
            {"DEFINITION_LD": definition_ld}
        )
    ...
    if symbols_has_symbol(symbols, "rand"):
        subst["INCLUDE_RAND"] = generate_include_rand(
            implementation_rand, target_search_path, definition_ld
        )
    # Symbol definitions.
    symbol_definitions_direct = generate_symbol_definitions_direct(
        symbols, symbol_prefix
    )
    ...
    if "vanilla" == compilation_mode:
        ...
        symbol_definitions_table = generate_symbol_definitions_table(
            symbols, symbol_prefix
        )
    ...
    # TODO: deprecated
    if elfling:
        elfling = executable_search(
            ["elfling-packer", "./elfling-packer"], "elfling-packer"
        )
        if elfling:
    ...
    # Find assembler.
    assembler = Assembler(
        executable_find(assembler, default_assembler_list, "assembler")
    )
    ...
    if 1 < len(abstraction_layer):
        raise RuntimeError(
            "conflicting abstraction layers detected: %s" % (str(abstraction_layer))
        )
    ...
    if output_file:
        ...
        if is_verbose():
            print(
                "Using output file '%s' after source file '%s'."
