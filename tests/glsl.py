...
def compress_file(compression, src, dst):
    ...
    wfd.close()
    print(
        "Wrote '%s': %i -> %i bytes" % (dst, os.path.getsize(src), os.path.getsize(dst))
    )
...
def find_executable(basename, pathname, path="."):
    ...
    if os.path.exists(new_path) and (
        os.path.realpath(new_path) != os.path.realpath(path)
    ):
...
def main():
    ...
    parser.add_argument(
        "source", default=[], nargs="*", help="Source file(s) to process."
    )
    ...
    if preprocessor:
        if not executable_check(preprocessor):
            raise RuntimeError(
                "could not use supplied preprocessor '%s'" % (preprocessor)
            )
    else:
