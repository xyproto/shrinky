...
{
  return static_cast<float>(shrinky_rand() & 0xFFFF) * ((1.0f / 65535.0f) * op);
}
...
GLuint create_shader(const char *source, GLenum type)
{
  ...
  shrinky_glShaderSource(ret, 1, &pretty_source_c_str, NULL);
#else
...
static GLuint program_attach(GLenum type, const char *source, GLuint pipeline, GLbitfield mask)
{
  ...
  GLuint ret = shrinky_glCreateShaderProgramv(type, 1, &pretty_source_c_str);

  shrinky_glUseProgramStages(pipeline, mask, ret);
...
static GLuint pipeline_create()
{
  ...
  shrinky_glGenProgramPipelines(1, &ret);
  shrinky_glBindProgramPipeline(ret);
...
{
  //shrinky_glDisable(GL_DEPTH_TEST);
...
#if defined(SHRINKY_GLESV2)
  shrinky_glUniform3fv(g_uniform_u, 4, g_uniform_array);
  {
    int8_t array[] =
    {
      -3, 1,
...
#else
  shrinky_glProgramUniform3fv(g_program_fragment, g_uniform_u, 4, g_uniform_array);
...
{
  shrinky();
  shrinky_SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO);
#if defined(SHRINKY_GLESV2) && !defined(SHRINKY_VIDEOCORE)
  ...
  g_uniform_u = shrinky_glGetUniformLocation(g_program_fragment, "u");
#endif
#else
#if defined(USE_LD)
  GlslProgram program;
  ...
  if(flag_record)
  {
    ...
    for(;;)
    {
      ...
      if(SDL_PollEvent(&event) && (event.type == SDL_EVENT_KEY_DOWN) && (event.key.key == SDLK_ESCAPE))
      {
        break;
      }
...
int SHRINKY_MAIN(int argc, char **argv)
{
